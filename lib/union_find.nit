...
#     s.add(1)
#     s.add(2)
#     assert not s.are_same(1,2)
#     s.union(1,2)
#     assert s.are_same(1,2)
#
# `are_same` is transitive, reflexive and symetric
#
#     s.add(3)
#     assert not s.are_same(1,3)
#     s.union(3,2)
#     assert s.are_same(1,3)
class DisjointSet[E: Object]
	...
	#     s.union_all([1,2,3])
	#     assert not s.are_all_same([2,3,4])
	#     s.union_all([1,4,5])
	#     assert s.are_all_same([2,3,4])
	fun are_all_same(es: Collection[E]): Bool
	do
	...
	# Construct a partitionning on a subset of elements of e
	#
	#     var s = new DisjointSet[Int]
	#     s.add_all([1,2,3,4,5,6])
	#     s.union(1,2)
	...
	# Combines the subsets of `e` and `f`
	# ENSURE: `are_same(e,f)`
	fun union(e,f:E)
	do
		var ne = find(e)
	...
	# Combile the subsets of all elements of `es`
	# ENSURE: `are_all_same(cs)`
...
private class DisjointSetNode
	# if parent == self then the note is a root
